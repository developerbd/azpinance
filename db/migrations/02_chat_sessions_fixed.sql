-- Chat Session Management Tables - FIXED VERSION
-- This migration safely creates tables even if some parts already exist
...
BEGIN
    ...
    DROP FUNCTION IF EXISTS archive_old_chat_sessions();
    ...
    DROP POLICY IF EXISTS "Users can update chat messages in own sessions" ON chat_messages;
    DROP POLICY IF EXISTS "Users can delete chat messages in own sessions" ON chat_messages;
...
-- Function to auto-archive old sessions
CREATE FUNCTION archive_old_chat_sessions()
RETURNS void AS $$
BEGIN
    UPDATE chat_sessions
    SET status = 'archived'
