...
export function ForexForm({ contacts, initialData, mode = 'edit' }: ForexFormProps) {
    ...
    useEffect(() => {
        const fetchAccounts = async () => {
            const { data } = await supabase.from('financial_accounts').select('id, name, currency').eq('category', 'receiving');
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <Select value={receivingAccountId} onValueChange={setReceivingAccountId} required>
                        <SelectTrigger>
